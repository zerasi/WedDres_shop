...
var IMG_LENGTH = 1.3;//图片最大1MB
var IMG_MAXCOUNT = 10;//最多选中图片张数
...
$("#btn_ImgUpStart").click(function () {
    ...
    $.get("../../product/deleteProductDesAllByPid.do",{"pid":product_id},function(data){
        if(!data.success){
            alert('删除失败')
            return;
        }
    ...
    for (var i = 0; i < $(".lookimg").length; i++) {
        ...
        NOWLOOK.index = i;
        console.log($(".lookimg").eq(i).attr("num"))
        //如果当前图片已经上传，则不再重复上传
        ...
        $.ajax({
            type: "POST",
            url: "../../product/editProductDes.do",
            data: { 'img_base': IMG_BASE, 'img_format': IMG_FOMATE, 'lookIndex': NOWLOOK.index ,"pid":product_id ,"bak1":i},//图片base64编码，图片格式（当前仅支持jpg,png,jpeg三种），图片对象索引
            dataType: "json",
            success: function (data) {
                if (data.success) {
                    //图片上传成功回调
                    var UPTIME = Math.ceil(Math.random() * 400) + 400;//生成一个400-800的随机数，假设进图条加载时间不一致
                    $(".lookimg").eq([data.message]).attr("ISUP", "1");//记录此图片已经上传
                    $(".lookimg").eq([data.message]).children(".lookimg_progress").eq(0).children("div").eq(0).animate({ width: "100%" }, UPTIME, function () {
                        $(this).css({"background-color":"#00FF00","opacity":"0.5"}).text('上传成功');
                    });
                }
                else {//图片未上传成功回调
                    $(".lookimg")[data.message].children(".lookimg_progress").eq(0).children("div").eq(0).css("width", "100%").css("background-color", "red").text("上传失败");
                }
            },
            error: function (err) {
                //服务器连接失败报错处理
